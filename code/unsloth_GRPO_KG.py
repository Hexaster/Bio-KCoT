...
def _call_judge_api(question, answer, prediction, evidence_graph):
    ...
    细粒度判题逻辑：
    打分范围：0 - 100 分
    最终奖励：score / 20.0 (映射到 0 - 5 分)
    """
    # 1. 快速过滤无效答案
    if prediction is None or len(str(prediction).strip()) == 0:
        return 0.0

    # 2. 构造包含评分标准的 Prompt
    prompt = f"""
    You are an objective evaluator. You need to assess the quality of the model's answer based on the provided Question, Ground Truth, and Knowledge Graph Evidence.

    [Evaluation Criteria]:
    Please score the answer from 0 to 100 based on the following scale:
    - **0-20 (Bad)**: The answer is completely wrong, irrelevant, or hallucinates facts contradicting the Knowledge Graph.
    - **21-50 (Poor)**: The answer touches on relevant topics but misses the core correct answer or contains significant factual errors.
    - **51-80 (Good)**: The answer is mostly correct and aligns with the Ground Truth. It may miss minor details from the Knowledge Graph or lack reasoning depth.
    - **81-100 (Excellent)**: The answer is perfectly correct, aligns with the Ground Truth, AND effectively utilizes the logical connections in the Knowledge Graph/Evidence.
    ...
    Please evaluate the Model Answer.
    First, briefly explain your reasoning (1-2 sentences).
    Then, output the final score inside double brackets, like [[85]].
    """
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.0,  # 保持 0.0 以确保评分稳定性
            max_tokens=256  # 稍微调大一点，允许裁判解释原因
        )
        content = response.choices[0].message.content

        # 打印裁判的评价，方便你调试观察
        print(f"Judge output: {content}")
        ...
        if match:
            ...
            # 5. 归一化到 0-5 分
            final_reward = score / 20.0
            return final_reward
        else:
            print(f"Warning: Judge did not return a valid score format. Content: {content}")
            return 0.0
...
training_args = GRPOConfig(
    ...
    save_strategy="steps",
    report_to="none",
    output_dir=str(path("paths.grpo_kg_output", env="GRPO_KG_OUTPUT")),
